...
int divide(int i, int j, vector <contact> &Agenda){
    ...
    int jj = -1;
    while(i < j){
        if(Agenda[i].getNume() > Agenda[j].getNume() || (Agenda[i].getNume() == Agenda[j].getNume() && Agenda[i].getPrenume() > Agenda[j].getPrenume())){
            iter_swap(Agenda.begin() + i, Agenda.end() + j);
            ii = ii ^ (-jj);
            jj = jj ^ (-ii);
            ii = ii ^ (-jj);
        }
...
//TRANSLATED STEFAN
void quicki(int st, int dr, vector <contact> &Agenda){
    if(st < dr){
